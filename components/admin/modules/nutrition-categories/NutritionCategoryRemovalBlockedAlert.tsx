...
type NutritionCategoryRemovalBlockedAlertProps = {
  open: boolean;
...
export default function NutritionCategoryRemovalBlockedAlert({
  open,
  onOpenChange,
  categoryName,
  reason,
}: NutritionCategoryRemovalBlockedAlertProps) {
  const displayName = categoryName?.trim() || 'This category';
  const detail =
    reason?.trim() ||
    'This food category is currently referenced in active records and cannot be removed.';
