...
type MovementExerciseRemovalBlockedAlertProps = {
  open: boolean;
...
export default function MovementExerciseRemovalBlockedAlert({
  open,
  onOpenChange,
  exerciseName,
  reason,
}: MovementExerciseRemovalBlockedAlertProps) {
  const displayName = exerciseName?.trim() || 'This exercise';
  const detail =
    reason?.trim() ||
    'This exercise is currently referenced in active records and cannot be removed.';
