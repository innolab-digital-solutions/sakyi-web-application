...
type NutritionItemRemovalBlockedAlertProps = {
  open: boolean;
...
export default function NutritionItemRemovalBlockedAlert({
  open,
  onOpenChange,
  itemName,
  reason,
}: NutritionItemRemovalBlockedAlertProps) {
  const displayName = itemName?.trim() || 'This item';
  const detail =
    reason?.trim() ||
    'This food item is currently referenced in active records and cannot be removed.';
