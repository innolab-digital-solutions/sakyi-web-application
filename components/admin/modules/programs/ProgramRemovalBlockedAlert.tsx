...
type ProgramRemovalBlockedAlertProps = {
  open: boolean;
...
export default function ProgramRemovalBlockedAlert({
  open,
  onOpenChange,
  programName,
  reason,
}: ProgramRemovalBlockedAlertProps) {
  const displayName = programName?.trim() || 'This program';
  const detail =
    reason?.trim() ||
    'This care program is currently referenced by active records and cannot be removed.';
