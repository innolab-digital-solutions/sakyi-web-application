...
- **Application (use cases)**: `services/` — auth, table fetch, and form submission. Owns orchestration and is the only layer that calls the HTTP client and endpoint config. Hooks (`useForm`, `useTable`) and context (`AuthContext`) call into services, not into `lib/api/client`.
...
**Decision**: Use an explicit application layer under `services/` so that presentation (context, hooks) does not depend on infrastructure.

- **`services/auth.ts`**: `checkSession()` and `logout()`. Use the HTTP client and admin auth endpoints. Consumed by `AuthContext`.
- **`services/table.ts`**: `fetchTablePage(endpoint, params)`. Builds query string and calls the HTTP client. Consumed by `useTable`.
- **`services/form.ts`**: `submitRequest(method, url, body?, options?)`. Dispatches to the appropriate HTTP method. Consumed by `useForm`.

Context and hooks import only from `@/services/*` (and from `@/types/api`, `@/config/paths` as needed). They do **not** import `@/lib/api/client` or `@/config/endpoints`. This keeps presentation and application boundaries clear and makes it easy to test or swap implementations by injecting or mocking services.
...
- **Presentation**: Context and hooks depend on `services/`, `config/paths`, and `types`. They do not depend on `lib/api/client` or `config/endpoints`.
